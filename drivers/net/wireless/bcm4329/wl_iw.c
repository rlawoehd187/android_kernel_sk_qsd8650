...
static int 
check_prescan(wl_join_params_t *join_params)
{
	int cnt = 0;
	int indx = 0;
	...
	while (buf) 
	{
		...
		for ( indx = 0;  indx < list->count ; indx++ ) 
		{
			...
			if ((dtoh32(bi->SSID_len) != join_params->ssid.SSID_len) || memcmp(bi->SSID,join_params->ssid.SSID,join_params->ssid.SSID_len))
				continue;

			if (!memcmp(&bi->BSSID, &join_params->params.bssid, ETHER_ADDR_LEN))
			{
	...
	if(!cnt) 
	{
		...
		for (;node;) {
			...
			if(!memcmp(node->bss_info->SSID, join_params->ssid.SSID, join_params->ssid.SSID_len) && !memcmp(&node->bss_info->BSSID, &join_params->params.bssid, ETHER_ADDR_LEN)) 
			{
