...
void INC_EXTENSION_001(ST_FIB_INFO* pFibInfo, ST_FIC_DB* pstFicDB)
{
  ...
  ST_TYPE0of1Long_INFO*	pLongInfo = INC_NULL;
	...
	for(ucIndex = 0; ucIndex < pstHeader->ITEM.bitLength-1;)
	{
		...
		{
			if(((pstFicDB->aucSubChID[ucLoop] & INC_BIT_MASK) == (ucSubChId & INC_BIT_MASK)) 
	...
	for(ucFrame = 0 ; ucFrame < pstHeader->ITEM.bitLength-1; )
	{
		...
		for(ucLoop = 0; ucLoop < pService->ITEM.bitNumComponent; ucLoop++, ucFrame+=2)
		{
			...
			case TMID_2 :
				...
				//nChanPos = INC_CHECK_SERVICE_CNT8(pstFicDB->aucSubChID, (INC_UINT8)pMscnFidc->ITEM.bitSubChld, pstFicDB->ucSubChCnt, INC_BIT_MASK);
				nChanPos = INC_CHECK_SERVICE_CNT32(pstFicDB->aulServiceID, ulServiceId, pstFicDB->ucSubChCnt);
				if(nChanPos != -1)
				{
			...
			default :
				...
				for(ucIndex = 0; ucIndex < (INC_INT32)pstFicDB->ucServiceComponentCnt; ucIndex++){
					...
					{
						if((pstFicDB->aucSubChID[ucSubCnt] & INC_BIT_MASK) != pstFicDB->aucSubChPackMode[ucIndex])  continue;
...
void INC_EXTENSION_003(ST_FIB_INFO* pFibInfo, ST_FIC_DB* pstFicDB)
{
	...
	for(ucIndex = 0 ; ucIndex < pstHeader->ITEM.bitLength-1; )
	{
		...
		{
			if(((pstFicDB->auiServicePackMode[ucLoop] & INC_BIT_MASK) == (pTypeInfo->ITEM.bitScid & INC_BIT_MASK)) && 
         				((pstFicDB->aucSubChPackMode[ucLoop] & INC_BIT_MASK) == ((INC_UINT8)pIdInfo->ITEM.bitSubChId & INC_BIT_MASK))) 
			{
		...
		if(ucLSFlag) {
			...
			if(cResult != -1){
				cResult = INC_CHECK_SERVICE_CNT8(pstFicDB->aucSubChID, 
					pstFicDB->aucSubChPackMode[cResult], pstFicDB->ucSubChCnt, INC_BIT_MASK);
				if(cResult != -1) {
			...
			if(!pstMscShort->ITEM.bitMscFicFlag){
				cResult = INC_CHECK_SERVICE_CNT8(pstFicDB->aucSubChID, 
					pstMscShort->ITEM.bitSUBnFIDCId, pstFicDB->ucSubChCnt, INC_BIT_MASK);
				if(cResult >= 0) {
					pstFicDB->aucServiceComponID[cResult] = pstMsc->ITEM.bitScIds;
					pstFicDB->aulServiceID[cResult] = ulSerId;
				}
